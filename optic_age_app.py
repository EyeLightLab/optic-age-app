...
def build_regional_vulnerability_plot(rnfl_shap: np.ndarray, gcipl_shap: np.ndarray):
    ...
    # 0° = Sup(12시), 시계 방향으로 3H(90°)=Nasal, 6H(180°)=Inferior, 9H(270°)=Temporal
    ax1.set_theta_zero_location("N")
    ax1.set_theta_direction(-1)

    num_gc = 6
    width_gc = 2 * np.pi / num_gc

    # GCIPL_FEATURES 순서:
    # [TempSup, Sup, NasSup, NasInf, Inf, TempInf]  (gc_vals[0]~gc_vals[5])
    # 플롯 상에서 12시부터 시계방향:
    # Sup → NasSup → NasInf → Inf → TempInf → TempSup
    gc_order_vals = [
        gc_vals[1],  # Sup
        gc_vals[2],  # NasSup
        gc_vals[3],  # NasInf
    ...
    gc_labels = ["Sup", "NasSup", "NasInf", "Inf", "TempInf", "TempSup"]

    theta_gc_centers = np.deg2rad(np.arange(0, 360, 360 / num_gc))

    for val, center in zip(gc_order_vals, theta_gc_centers):
        start = center - width_gc / 2.0
        ax1.bar(
            start,
            1.0,
            width=width_gc,
    ...
    ax1.set_xticks(theta_gc_centers)
    ...
    # 0° = 12H(위), 시계 방향으로 3H(90°), 6H(180°), 9H(270°)
    ...
    # rnfl_vals 인덱스: 0~11 = 1H~12H
    # 플롯 순서: 12H, 1H, 2H, ..., 11H  (6H가 180° 위치)
    rnfl_order_vals = [rnfl_vals[11]] + [rnfl_vals[i] for i in range(11)]
    hour_labels = ["12H"] + [f"{i}H" for i in range(1, 12)]

    theta_centers = np.deg2rad(np.arange(0, 360, 360 / num))

    for val, center in zip(rnfl_order_vals, theta_centers):
